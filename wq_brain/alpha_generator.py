...
class AlphaGenerator:
    ...
    def _init_regular_templates(self) -> List[AlphaTemplate]:
        ...
        return [
            ...
            AlphaTemplate(
                name="mean_reversion",
                expression="rank((close - ts_mean(close, {window})) / (ts_std_dev(close, {window}) + 0.001))",
            ...
            AlphaTemplate(
                name="volatility_low",
                expression="-rank(ts_std_dev(ts_returns(close, 1), {window}))",
                category="volatility",
            ...
            AlphaTemplate(
                name="vwap_reversion",
                expression="rank((vwap - close) / (ts_std_dev(close, {window}) + 0.001))",
                category="mean_reversion",
            ...
            AlphaTemplate(
                ...
                expression="-rank(ts_returns(close, {delay})) * (1 - rank(ts_std_dev(ts_returns(close, 1), {window})))",
                category="reversal",
                description="低波权重反转",
    ...
    def _init_power_pool_templates(self) -> List[AlphaTemplate]:
        ...
        return [
            ...
            AlphaTemplate(
                ...
                expression="rank(ts_delta(close, {delay}) / (ts_std_dev(close, {std_window}) + 0.001)) * rank(volume)",
                category="momentum",
    ...
    def _init_superalpha_templates(self) -> List[AlphaTemplate]:
        ...
        return [
            ...
            AlphaTemplate(
                name="triple_screen",
                expression="(rank(ts_delta(close, {delay1})) + rank(ts_delta(volume, {delay2})) + rank(ts_std_dev(close, {window}))) / 3",
                category="combined",
            ...
            AlphaTemplate(
                ...
                expression="rank((close - ts_mean(close, {window1})) / (ts_std_dev(close, {window1}) + 0.001)) * (1 - rank(ts_std_dev(close, {window2})))",
                category="combined",
